...
req=[
 ...
 'app/src/main/assets/qa_nav.html',
...
for k in ['MODE_HOLD','MODE_PULSE','MODE_PAGE','MODE_REPEAT','hitJoystick','setMoveState(','releaseJoystick(','pulse(','hardResetInputs(','hasBundledGame(','onWindowFocusChanged','lastPulseTimes','pulseReleaseTasks','ARECT:','JOY:','repeatKeyCode(','reassertMovement(','startAttackRepeat(','containsButton(','shouldBlockTopNavigation(','BLOCK_NAV:','qa_nav.html']:
    assert k in s,k
...
assert 'if(last != null && now - last < 38L) return' in s
...
assert 'qa_input' in s and 'qa_ruffle' in s and 'qa_nav' in s and 'REVA_JS' in s and 'qa_input.html' in s and 'qa_ruffle.html' in s and 'qa_nav.html' in s
...
for k in ['KEYDOWN:X:','KEYUP:X:','KEYDOWN:A:','KEYUP:A:','ATTACK_START','ATTACK_STOP','PULSE:A','--ez qa_input true','--ez qa_ruffle true','--ez qa_nav true','RUFFLE_QA_READY','BLOCK_NAV','XRECT:','ARECT:','JOY:']:
    assert k in smoke,k
app=(r/'app/build.gradle').read_text()
assert "versionCode 13" in app and "versionName '1.3'" in app
assert "noCompress += ['swf', 'wasm']" in app
print('PASS: project structure + v1.3 version')
print('PASS: joystick + DNF-mobile-inspired skill arc + two skill pages')
print('PASS: input manager guards: movement repeat/resync, repeat-attack pulses, fixed skill pulse, release-all lifecycle')
print('PASS: virtual HTTPS local origin + bundled-game startup + external navigation guard')
